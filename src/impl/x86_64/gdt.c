...
void init_gdt() {
    ...
    set_gdt_gate(1, 0, 0, 0x9A, 0xA0); // Notice the 0xA0 for 64-bit granularity

    // Kernel data segment (read/write)
    set_gdt_gate(2, 0, 0, 0x92, 0xCF);

    // User code segment (64-bit, execute, read)
    set_gdt_gate(3, 0, 0, 0xFA, 0xA0); // 64-bit user mode code segment

    // User data segment (read/write)
    set_gdt_gate(4, 0, 0, 0xF2, 0xCF);
