...
void initialize_PS2_mouse() {
    ...
    out_port_b(0x64, 0xA8);  // Send command 0xA8 to enable the PS/2 mouse

    // Step 2: Request the current command byte from the PS/2 controller
    out_port_b(0x64, 0x20);  // Send command 0x20 to read the PS/2 controller command byte
    input = in_port_b(0x60);  // Read the command byte from port 0x60
    ...
    out_port_b(0x64, 0x60);  // Send command 0x60 to prepare for writing the command byte
    out_port_b(0x60, input);  // Write the modified command byte back to port 0x60
    ...
    ack = in_port_b(0x60);  // Read the acknowledgment byte from the mouse
    if (ack != 0xFA) {
        in_port_b(0x60);
